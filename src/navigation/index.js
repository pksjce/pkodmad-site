...
export class Navigation extends React.Component {
  ...
  render() {
    const { stack: currentStack, back } = this.state.stackStack[
      this.state.stackStack.length - 1
    ];
    console.log("rerear");
    return (
      <div>
...
export class LocalNav extends React.Component {
  ...
  renderItem = (item, onClick) => (
    <NavigationItem onClick={onClick} item={item} />
  );
  render() {
    return yeild({
      renderLocalNav: (goBack, updateStack, stack, back, loading) => {
        return (
          <div>
            {back ? (
              <NavigationItem
                onClick={this.onClick(back, goBack)}
                item={back}
                isBack
              />
            ) : null}
            {loading ? (
              ...
              stack.map(item =>
                this.renderItem(item, this.onClick(item, updateStack))
              )
            )}
          </div>
        );
